...
import { createScenario } from './generate-scenario.js';
...
import('paper').then(({ default: paper }) => {
  ...
  window.onload = function () {
    ...
    // Generate Scenario
    importScenario(createScenario(urlScen));
...
const importScenario = function (data) {
  window.genShipsAfloat = _.cloneDeep(data.genShipsAfloat);
  window.genCentre = data.center;
  let importedShipsAfloat = data.genShipsAfloat;
  ...
  shipsAfloat = importedShipsAfloat;
...
function sendScenario(ships, resvis, elevation, center) {
  const staffAnswer = $('#userInsertedText').val();
  const tagArray = [];
  const tagNo = [1, 2, 3, 4, 5];
  tagNo.map((val) => {
    if ($(`#tag${val}`).val().length > 0) {
      tagArray.push($(`#tag${val}`).val().toString());
    }
  });
  // //Build data object
  const data = {
    scenarioObject: {
      genShipsAfloat: ships,
      elevation: elevation,
      resVis: resvis,
      center: center,
    },
  ...
  // // post data
  reqData('post', `${APIURL}api/v1/scenarios`, data)
    .then((res) => {
      $('#regForm').hide();
      $('#response').html(
        'Staff Answer:' +
          res.data.staff_answer +
          '</br>' +
          'Tags: ' +
          res.data.tags +
          '</br>' +
...
$('#submitScenario').on('touchstart mousedown', function (e) {
  e.stopImmediatePropagation();
  e.preventDefault();
  sendScenario(
    window.genShipsAfloat,
    window.resVis,
    window.elevation,
    window.genCentre
  );
